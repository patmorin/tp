...
The \defin{length} of a path is the number of edges in it. For any two vertices $v,w\in V(G)$, $\dist_G(v,w)$ denotes the minimum length of a path in $G$ that contains $v$ and $w$, or $\dist_G(v,w):=\infty$ if $v$ and $w$ are in different connected components of $G$. For any $R\subseteq V(G)$, the \defin{diameter} of $R$ is $\diam_G(R):=\max\{\dist_G(v,w):v,w\in R\}$.
...
The \defin{depth} of a vertex $v$ in a rooted tree $T$ is the length of the path $P_T(v)$ from $v$ to the root of $T$.  Each vertex $a\in V(P_T(w))$ is an \defin{ancestor} of $v$, and $v$ is a \defin{descendant} of each vertex in $V(P_T(w))$.  We say that a set $B\subseteq V(T)$ is \defin{unrelated} if no vertex of $B$ is an ancestor of any other vertex in $B$.

For each $h\in\N$, let $T_h$ denote the complete binary tree of height $h$ with root $r$; i.e., the rooted ordered tree with $2^h$ leaves, each having depth $h$ and in which each non-leaf vertex has exactly two children, one \defin{left child} and one \defin{right child}.  Note that the ordering of $T_h$ induces an ordering on any unrelated set $B\subseteq V(T_h)$, which we refer to as the \defin{left-to-right ordering}.  Specifically, $v$ appears before $w$ in the left-to-right ordering of $B$ if and only there exists a common ancestor $a$ of both $v$ and $w$ such that the path from $a$ to $v$ contains the left child of $a$ and the path from $a$ to $w$ contains the right child of $a$.
...
We are now ready to show that for any tree $T$ and any path $P$, if $G_{h}\subsetcong T\boxtimes P\boxtimes K_c$ then $c\ge 2^{\Omega(\log h)}$.  By \cref{ding_translation}, this is enough to prove \cref{main_thm}.

% \todo[inline]{This is probably the place to write a proof outline.}

We now outline our proof strategy.  Recall \cref{partitions_vs_products} which states that if $G_h\subsetcong T\boxtimes P\boxtimes K_c$ then $G_h$ has a tree-partition $\mathcal{T}:=\{B_x:x\in V(T)\}$ and a path-partition (i.e., layering) $\mathcal{P}:=\{P_y:y\in V(P)\}$ with $|B_x\cap P_y|\le c$ for each $(x,y)\in V(T)\times V(P)$. Since $G_h$ has diameter $2h$, \cref{diameter_spread} implies that $|B_x|\le c(2h+1)$ for each $x\in v(T)$, and we make use of this upper bound on the sizes of parts in what follows.

Our strategy is to first show that $\mathcal{T}$ has some part $B_{x_1}$ that contains an unrelated set $\mathcal{R}_1$ of small depth and size at least $\alpha h$ for some constant $\alpha >0$.  Next we show that $x_1$ has a neighbour $x_2$ such that $B_{x_2}$ contains an unrelated set $\mathcal{R}_2$ that is smaller than $\mathcal{R}_1$ by a factor of at most $b/2$ (for some small $b>2$), but whose elements can be paired off so that each pair has a common ancestor in $\mathcal{R}_1$ that is at distance at most $a$ from both vertices in the pair (for some small $a>0$).  By iterating this argument $t$ times, we obtain a sequence of unrelated sets $\mathcal{R}_1,\ldots,\mathcal{R}_{t+1}$ where the elements in each $\mathcal{R}_i$ are partitioned into parts of size $2^{i-1}$ so that each part $R$ has a common ancestor in $\mathcal{R}_1$ that is at distance at most $a(i-1)$ from all elements in $R$.\todo{$R$ is a part in the partition of $\mathcal{R}_i$}  Therefore $\mathcal{R}_{t+1}\subseteq B_{x_{t+1}}$ contains an unrelated set of size $2^{t}$ whose diameter is at most $2at$.  \cref{diameter_spread} then implies that $|B_{x_{t+1}}\cap P_y|\ge 2^{t}/(2at+1)$ for some $y\in V(P)$.  For any fixed $c$, and any $t\in\omega(1)$, this already provides the desired contradiction.
To obtain the $2^{\Omega(\sqrt{\log h})}$ in \cref{main_thm}, we choose $c=2^{O(\sqrt{\log h})}$.
% By choosing  $t\approx\log_b |R_1|\ge \log_{b}(\alpha h)$, this implies that $c\ge |B_{x_{t+1}}\cap P_y|\gtrapprox 2^{\log_{b}(\alpha h)}/2a(\log_{b}(\alpha h))=h^{\alpha/\log b-o(1)}$.\todo{Verify that this last sentence still agrees with the current proof.}
...
\begin{inparaenum}[(i)]
  ...
  \item If $a_i$ precedes $a_j$ in the left-to-right order of $\{a_1,\ldots,a_q\}$ then every element of $R_i$ precedes every element of $R_j$ in the left-to-right order of $\cup\mathcal{R}$.
\end{inparaenum}
