...
A set $\mathcal{R}:=\{R_1,\ldots,R_r\}$ of subsets of $V(G_0)$ is \defin{$k$-great}\todo{pick a better name} if it has the following properties:
...
% \begin{lem}
%   Let $\mathcal{R}:=\{R_1,\ldots,R_r\}$ be a $k$-great that is compatible with some $S\supseteq \cup \mathcal{R}$ and such that $S$ contains at most $w$ elements of depth $i$ in $G_0$, for each $i\in\{0,\ldots,h\}$.  Then $G-\cup\mathcal{R}$ contains a component $C$
% \end{lem}
