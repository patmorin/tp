...
\begin{proof}
  ...
  By \cref{startup}, with $\alpha:= 1/5$, $T$ contains a node $x_1$ such that $B_{x_1}$ contains an unrelated set $R$ of size $q_1:=|R|\ge h/25c$ where each vertex in $R$ has height at least $m:=m_1:=h/5$.  Let  $\mathcal{R}_1':=\{\{v\}:v\in R\}$.  By definition $\mathcal{R}_1'$ is a $(1,0,m)$-compact set.  $\mathcal{R}_1'$ will be the first in a sequence of sets $\mathcal{R}'_1,\ldots,\mathcal{R}'_{t+1}$. For each $i\in\{1,\ldots,t\}$, $\mathcal{R}'_i$ satisfies the following properties:
  \begin{enumerate}[(i)]
     \item $\mathcal{R}'_i$ is a $(2^{i-1},(i-1)(\floor{\log_2(ch)}+2,m)$ compact set.
     \item $q_i:=|\mathcal{R}'_i| \ge q_1/(10c)^{i-1} - 2\sum_{j=0}^{i-2}(10c)^{-j} > q_1/(10c)^{i-1} - 3$.
     \item Each vertex in $\cup\mathcal{R}'_i$ has height at least $m_i:=h/5-(i-1)(\floor{\log_2(ch)}+2)$.
     \item There exists $x_i\in V(T)$ such that $\cup\mathcal{R}'_i\subseteq B_{x_i}$.
  \end{enumerate}
  Let $t:= \min\{t_1,t_2\}$ where $t_1:=\floor{\log_{10c}(q_1/3)}$ and $t_2:=\floor{h/(10\log_2(ch)+2)}-1$. Observe that, since $c<2^{\sqrt{\log_2 h}}$,  $t_1\ge \log_{10c}(h/75c)\in\Omega(\log_c h)\subseteq \Omega(\sqrt{\log h})$ and that $t_2\ge h/10(\sqrt{\log h} + \log_2 h + 2)-1\in\Omega(h/\log h)$.  Therefore $t\in\Omega(\sqrt{\log h})$. These specific values are chosen for the following reasons:
  \begin{compactenum}[(i)]
    \item Since $t\le t_1$, $q_{t+1} > q_1/(10c)^{t_1} - 3  \ge 0$.
    \item Since $t\le t_2$, $m_i \ge h/5-(t_2+1)(\floor{\log_2(ch)}+2) \ge h/10$ for each $i\in\{2,\ldots,t+1\}$.
  \end{compactenum}
  We now describe how to obtain $\mathcal{R}'_{i+1}$ from $\mathcal{R}'_i$ for each $i\in\{1,\ldots,t\}$.

  By \cref{compatible_set} (applied to $\mathcal{R}_i'$ and $B_{x_i}$), $T_h$ contains a $(2^i,i\log_2(ch)+2,m)$-compact set $\mathcal{R}_i$ of size $q_i$ that is compatible with $B_{x_i}$. For each $v\in\cup\mathcal{R}_i$, $v$ has height at least $m_{i+1}:=m_i-(\floor{\log_2(ch)}+2)\ge h/5-i(\floor{\log_2(ch)}+2)$ and $T_h-B_{x_1}$ contains a path $P_v$ from $v$ to a leaf of $T_h$.  The union of the paths in $P_{h-m_i+1},\ldots,P_{h}$ and the paths in $\mathcal{C}_i:=\{P_v:v\in\cup\mathcal{R}_1\}$ contains a subgraph $G'$ isomorphic to a graph that can be obtained from $G_{2^{i}q_i\times m_{i+1}}$ by subdividing horizontal edges.  By \cref{grid_connectivity} applied to $G'$ with $S=B_{x_i}$, some component $X$ of $G_h-B_{x_1}$ contains  $q_i'\ge 2^{i}q_im_{i+1}/ch\ge 2^iq_i/10c$ consecutive columns $C_1,\ldots,C_{q_i'}$ of $G'$.\todo{Check for strict inequality here.}

  Since $\cup\mathcal{R}_i$ is unrelated, it has a left to-right-order. This order defines a total order $\prec$ on the paths in $\mathcal{C}_i$, in which $P_v\prec P_w$ if and only if $v$ precedes $w$ in left-to-right order. The resulting total order $(\prec,\mathcal{C}_i)$ corresponds to the order of the columns in $G'$ and each part in $\mathcal{R}_i$ corresponds to $2^i$ consecutive columns of $G'$. There are at most two parts $R\in\mathcal{R}_i$ such that $0 < |R\cap (C_1\cup\cdots\cup C_{q_i'})| < |R|$.  These two parts account for at most $2(2^{i}-1)$ of the columns in $C_1,\ldots, C_{q_i'}$.  Therefore, the number of parts of $\mathcal{R}_i$ completely contained in $C_1\cup\cdots\cup C_{q_i'}$ is
  \begin{align*}
  ...
  \end{align*}
  We define $\mathcal{R}_{i+1}'$ as the set of parts in $\mathcal{R}_i$ that are completely contained in $C_1\cup\cdots\cup C_{q_i'}$.  This completes the definition of $\mathcal{R}_1',\ldots,\mathcal{R}_{t+1}'$.
  This definition implies that, $\mathcal{R}_{t+1}'$ is a $(2^t,t(\log_2(ch)+2),m)$-compact set $\mathcal{R}_{t+1}'\subseteq B_{x_t}$ of size at $q_{t+1} \ge q_1/(10c)^t-2 > 0$.  In particular $\mathcal{R}_{t+1}'$ is non-empty.
  ...
  \begin{align*}
    ...
    %   & \text{(since $c<2^{x})}\\
     = 2^{\Omega(\sqrt{\log h})} \enspace . \qedhere
      % & \text{(since $c\le 2^{\sqrt{\log h}}$ and $q_1\ge h/25c$).} & \qedhere
