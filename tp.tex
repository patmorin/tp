...
\begin{proof}
  By \cref{dingy}, $H$ has a tree-partition $\mathcal{T}:=\{B_x:x\in V(T)\}$ of width at most $24\Delta t$. By \cref{partitions_vs_products}, $H \subsetcong T\boxtimes K_{24\Delta t}$.  Therefore, $G\subsetcong T\boxtimes K_{24\Delta t}\boxtimes P\boxtimes K_c \cong T\boxtimes P\boxtimes K_{24c\Delta t}$.
\end{proof}
...
\begin{lem}\label{small_depth_separator}
  For all $h\in\N$, if $S\subseteq V(G_h)$ is such that $G_h-S$ has no component with more than $|V(G_h)|/2$ vertices then, for each $i\in\{i_0,\ldots,h\}$, $S$ contains at least one vertex of depth $i$, where $i_0=\ceil{\max\{2\log|S|+2, \log(2+(h+1)|S|)-1}$.
\end{lem}

\begin{proof}
  Let $C$ be the vertex set of a component of $G_h-S$ that maximizes $C\cap V(P_h)$. For each $i\in\{0,\ldots,h\}$, let $C_i:=C\cap V(P_i)$ and let $S_i:=S\cap V(P_i)$. We will show that, for each $i\ge i_0$, $C_i$ is non-empty but does not contain all $2^i$ vertices in $P_i$.  Therefore $S_i\supseteq N_G(C_i)\cap V(P_i)\neq\emptyset$ for each $i\in\{\ceil{\gamma(1+h\log|S|)},\ldots,h\}$.

  For each $i\in\{0,\ldots,h-1\}$, the vertices in $C_{i+1}$ are adjacent to at least $|C_{i+1}|/2$ vertices of $P_{i}$, so $|C_i|\ge |C_{i+1}|/2 - |S_i|$.  Iterating this inequality $h-i$ times gives $|C_i|\ge |C_h|/2^{h-i}-\sum_{j=i}^{h-1}|S_j|\ge |C_h|/2^{h-i}-|S|$.  The vertices of $|S|$ partition $V(P_h)\setminus S$ into at most $|S|+1$ connected components.  Since $C$ is chosen to maximize $|C_h|$, $|C_h| \ge (2^h-|S|)/(|S|+1) > 2^{h}/(|S|+1) - 1$.  Therefore,
  \begin{equation}
    |C_i|\ge \frac{|C_h|}{2^{h-i}} - |S| > \frac{2^h/(|S|+1)-1}{2^{h-i}} - |S|
    \ge 2^{i-\log(|S|+1)}-|S|-1
  \end{equation}
  for all $i\in\{0,\ldots,h\}$.  This establishes that $|C_i|>0$ for all $i\ge 2\log |S|+2$.

  For each $i\in\{0,\ldots,h-1\}$, the vertices in $C_i$ are adjacent to at least $2|C_i|$ vertices of $P_{i+1}$, so $|C_{i+1}|\ge 2|C_i|-|S_{i+1}|$.  Iterating this $h-1$ times gives:
  \begin{equation}
  ...
  \end{equation}
  However, $2^h > |V(G_h)|/2 \ge |C| \ge (\ref{crux})$.  Rearranging terms and using the fact that $i^*\le h$, shows that \cref{crux} can only be satisified when $i^* < \log((h+2)|S|)$.  Therefore, $|C_i|< 2^i$ for all $i \ge \log((h+2)|S|)$. \qedhere
  %
