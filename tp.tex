...
\begin{thm}[\citet{ding.oporowski:some}]\label{dingy}
  If $H$ is a graph with maximum degree $\Delta$ and treewidth $t$, then $H$ has a tree-partition of width at most $24\Delta t$.\todo{DW suggests writing "treewidth less than $t$" here, but that's not accurate. If you mean "treewidth at most $t$", that seems redundant}
\end{thm}
...
A set $\mathcal{R}:=\{R_1,\ldots,R_q\}$ of subsets of $V(T_h)$ is \defin{$(k,\ell,m)$-great}\todo{pick a better name} if it has the following properties:
