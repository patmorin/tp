...
\begin{lem}\label{small_depth_separator}
  There exists a function $i_0:\R_{>0}\to\N\in O(\log n)$ such that the following is true, for any $h\in\N$.  Let $S\subseteq V(G_h)$ have $|S|\le ch$ and $G_h-S$ have no component with more than $|V(G_h)|/2$ vertices.  Then, for each $i\in\{i_0(ch+1),\ldots,h\}$, $S$ contains at least one vertex of depth $i$.
\end{lem}

\begin{proof}
  Let $C$ be the vertex set of a component of $G_h-S$ that contains the largest number of leaves of $T_h$.  For each $i\in\{0,\ldots,h\}$, let $C_i:=C\cap V(P_i)$. We will show that, for each $i\in\{i_0(ch+1),\ldots,h\}$, $0< |C_i| < 2^i$, so that $S\cap V(P_i)\supset N_G(C_i)\cap V(P_i)\neq\emptyset$.
