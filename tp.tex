...
If we use $G\subseteq A$ to mean that $G$ is isomorphic to a subgraph of $A$ then why do we use $G\cong A$ to mean that $G$ is isomorphic to $A$?  Why don't we use $G\scong A$? 
\end{minipage}
}
}
...
% Our strategy is to first show that in any tree partition of $G_h$ some node contains a unrelated set of size at least $\alpha h$ for some fixed $\alpha >0$.  Next,  
